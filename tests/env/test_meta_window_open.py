...
def test_create_meta_window_open():
    env = create_mj_env(OmegaConf.create({"name": "MetaWindowOpen"}))
...
def test_ground_truth_prediction_meta_window_open():
    env = create_mj_env(OmegaConf.create({"name": "MetaWindowOpen"}))
    prediction = GroundTruth(OmegaConf.create({"name": "MetaWindowOpen"}))
    state = env.reset()
